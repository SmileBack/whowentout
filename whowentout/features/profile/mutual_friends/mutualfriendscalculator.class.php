...
class MutualFriendsCalculator
{
    ...
    function compute($user_a_id, $user_b_id)
    {
        ...
        list($sql, $params) = $this->mutual_friends_query($user_a_id, $user_b_id);

//        $this->database->table('users')->prefetch($sql, $params);

        $query = $this->database->query_statement($sql, $params);
        $query->execute();
    ...
    {
        $sql = "SELECT users.id AS id FROM user_friends AS a
